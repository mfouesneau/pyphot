...
class Filter(object):
    ...
    def lphot(self):
        ...
        with Vega() as v:
            f_vega = self.get_flux(v.wavelength, 
                                   v.flux, 
                                   axis=-1)
            f_lamb_vega = self.get_flux(v.wavelength, 
                                        v.wavelength.value * v.flux, 
                                        axis=-1)
            f_lamb2_vega = self.get_flux(v.wavelength, 
                                         v.wavelength.value ** 2 * v.flux, 
                                         axis=-1)
        if 'photon' in self.dtype:
    ...
    def AB_zero_Jy(self):
        ...
        f = 1e5 / c * self.lpivot.value ** 2 * self.AB_zero_flux.value
        return f * Unit('Jy')
    ...
    def Vega_zero_mag(self):
        ...
        with Vega() as v:
            f_vega = self.get_flux(v.wavelength, v.flux.value, axis=-1)
    ...
    def Vega_zero_Jy(self):
        ...
        c = 1e-8 * constants.c.to('m/s').value
        f = 1e5 / c * self.lpivot.value ** 2 * self.Vega_zero_flux.value
        return f * Unit('Jy')
    ...
    def ST_zero_Jy(self):
        ...
        c = 1e-8 * constants.c.to('m/s').value
        f = 1e5 / c * self.lpivot.value ** 2 * self.ST_zero_flux.value
        return f * Unit('Jy')
