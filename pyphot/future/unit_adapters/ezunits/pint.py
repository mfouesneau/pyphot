...
class DimensionalityError(ValueError):
    ...
    def __str__(self):
        if self.dim1 or self.dim2:
            ...
            dim1 = ""
            dim2 = ""
        return "Cannot convert from '{}'{} to '{}'{}".format(
            self.units1, dim1, self.units2, dim2
        )
...
class UnitsContainer(dict):
    ...
    def __format__(self, spec):
        if spec == "!s" or spec == "":
            ...
            tmp = (
                self._formatter(r" \cdot ", "^[{:n}]", True, True)
                .replace("[", "{")
                .replace("]", "}")
            )
            if "/" in tmp:
...
class UnitRegistry(object):
            ...
            try:
                ...
                if modifiers:
                    self.add_unit(
                        "delta_" + name,
                        value,
                        tuple("delta_" + item for item in aliases),
                    )
    ...
    def get_alias(self, name):
        ...
        return self._PREFIXES.preferred_alias.get(
            prefix, prefix
    ...
    def _to_canonical(self, candidate):
        ...
        if prefix:
            ...
            self.add_unit(
                prefix + unit_name,
                self.Quantity(self._PREFIXES[prefix], unit_name),
                (alias,),
            )
    ...
    def _parse_candidate(self, candidate):
        ...
        for suffix, prefix in itertools.product(
            self._SUFFIXES.keys(), self._PREFIXES.keys()
        ):
            if candidate.startswith(prefix) and candidate.endswith(suffix):
                ...
                if unit_name in self._UNITS:
                    yield (
                        self._PREFIXES.get_aliased(prefix),
    ...
    def _parse_expression(self, input):
        ...
        return eval(
            untokenize(result),
            {"__builtins__": None},
            {"REGISTRY": self._UNITS, "Q_": self.Quantity, "U_": UnitsContainer},
        )
    ...
    class _Quantity(object):
        ...
        def __new__(cls, value, units=None, offset=0, log_base=0):
            if units is None:
                ...
                raise TypeError(
                    "units must be of type str, Quantity or UnitsContainer; not {}.".format(
                        type(units)
                    )
                )
        ...
        def ito(self, other=None):
            ...
            if not factor.unitless:
                raise DimensionalityError(
                    self.units, other.units, self.dimensionality, other.dimensionality
                )
        ...
        def __mul__(self, other):
            if isinstance(other, self.__class__):
                return self.__class__(
                    self._magnitude * other._magnitude, self._units * other._units
                )
            else:
        ...
        def __rfloordiv__(self, other):
            if isinstance(other, self.__class__):
                return self.__class__(
                    other._magnitude // self._magnitude, other._units / self._units
                )
            else:
        ...
        #: original.
        __copy_units = (
            "clip compress conj conjugate copy cumsum diagonal flatten "
            ...
            if item.startswith("__array_"):
                if isinstance(self._magnitude, ndarray):
                    ...
                    raise AttributeError(
                        "__array_* attributes are only taken from ndarray objects."
                    )
            try:
                return functools.partial(
                    self.__numpy_method_wrap, getattr(self._magnitude, item)
                )
            except AttributeError:
                raise AttributeError(
            ...
            try:
                if isinstance(value, self.__class__):
                    factor = self.__class__(
                        value.magnitude, value.units / self.units
