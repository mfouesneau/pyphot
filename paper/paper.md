...
Another notable package is `sedpy`, which initially intended to provide a suite to forward model photometry is now tool to calculate photometry through an extensive list of passbands. `pyphot`
...
We thank the two JOSS reviewers and Chris Lovell, for their suggestions to improve the code and the manuscript, as well as the JOSS editor, Josh Borrow.
